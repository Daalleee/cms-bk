            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse ($areas as $area)
                    <div class="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition group">
                        <div class="w-12 h-12 bg-indigo-100 text-indigo-600 rounded-lg flex items-center justify-center mb-6 group-hover:bg-indigo-600 group-hover:text-white transition overflow-hidden">
                            @if($area->ikon)
                                <img src="{{ asset('storage/' . $area->ikon) }}" alt="{{ $area->nama_area }}" class="w-full h-full object-contain p-2 group-hover:brightness-0 group-hover:invert transition">
                            @elseif($area->gambar)
                                <img src="{{ asset('storage/' . $area->gambar) }}" alt="{{ $area->nama_area }}" class="w-full h-full object-cover">
                            @else
