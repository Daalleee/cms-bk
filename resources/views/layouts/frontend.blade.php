...
<head>
    ...
    <style>
        ...
        .reveal-visible-left { opacity: 1; transform: translateX(0); }
        .reveal-right { opacity: 0; transform: translateX(40px); transition: opacity 0.7s ease-out, transform 0.7s ease-out; }
        .reveal-visible-right { opacity: 1; transform: translateX(0); }
        .reveal-scale { opacity: 0; transform: scale(0.9); transition: opacity 0.6s ease-out, transform 0.6s ease-out; }
        .reveal-visible-scale { opacity: 1; transform: scale(1); }
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-12">
                <div class="col-span-1 md:col-span-2">
                    ...
                    <div class="flex space-x-4">
                        @if($kontak && $kontak->youtube && $kontak->tampilkan_youtube)
                            <a href="https://youtube.com/@{{ $kontak->youtube }}" target="_blank" class="w-10 h-10 rounded-xl bg-gray-800 flex items-center justify-center text-gray-400 hover:bg-red-600 hover:text-white transition-all duration-300">
                        ...
                        @if($kontak && $kontak->instagram && $kontak->tampilkan_instagram)
                            <a href="https://instagram.com/{{ $kontak->instagram }}" target="_blank" class="w-10 h-10 rounded-xl bg-gray-800 flex items-center justify-center text-gray-400 hover:bg-pink-600 hover:text-white transition-all duration-300">
                        ...
                        @if($kontak && $kontak->facebook && $kontak->tampilkan_facebook)
                            <a href="https://facebook.com/{{ $kontak->facebook }}" target="_blank" class="w-10 h-10 rounded-xl bg-gray-800 flex items-center justify-center text-gray-400 hover:bg-blue-600 hover:text-white transition-all duration-300">
                                <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                            </a>
                        @endif
                        @if($kontak && $kontak->twitter && $kontak->tampilkan_twitter)
                            <a href="https://twitter.com/{{ $kontak->twitter }}" target="_blank" class="w-10 h-10 rounded-xl bg-gray-800 flex items-center justify-center text-gray-400 hover:bg-gray-900 hover:text-white transition-all duration-300">
                        ...
                        @if($kontak && $kontak->tiktok && $kontak->tampilkan_tiktok)
                            <a href="https://tiktok.com/@{{ $kontak->tiktok }}" target="_blank" class="w-10 h-10 rounded-xl bg-gray-800 flex items-center justify-center text-gray-400 hover:bg-gray-900 hover:text-white transition-all duration-300">
                ...
                <div>
                    ...
                    <ul class="space-y-3 text-gray-400">
                        <li><a href="/#hero" class="hover:text-white transition flex items-center gap-2 group"><span class="w-0 group-hover:w-4 h-0.5 bg-indigo-400 transition-all duration-300"></span>Beranda</a></li>
                        <li><a href="/#tentang" class="hover:text-white transition flex items-center gap-2 group"><span class="w-0 group-hover:w-4 h-0.5 bg-indigo-400 transition-all duration-300"></span>Tentang Kami</a></li>
                        <li><a href="/#alur" class="hover:text-white transition flex items-center gap-2 group"><span class="w-0 group-hover:w-4 h-0.5 bg-indigo-400 transition-all duration-300"></span>Alur Penanganan</a></li>
                        <li><a href="/#area" class="hover:text-white transition flex items-center gap-2 group"><span class="w-0 group-hover:w-4 h-0.5 bg-indigo-400 transition-all duration-300"></span>Area Kecanduan</a></li>
                        <li><a href="/#testimoni" class="hover:text-white transition flex items-center gap-2 group"><span class="w-0 group-hover:w-4 h-0.5 bg-indigo-400 transition-all duration-300"></span>Testimoni</a></li>
                    </ul>
                </div>
                <div>
                    ...
                    <ul class="space-y-4 text-gray-400">
                        <li class="flex items-start gap-3 group">
                            <div class="w-10 h-10 rounded-xl bg-gray-800 flex items-center justify-center text-indigo-400 shrink-0 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                            </div>
                            <span class="pt-2">{{ $kontak->alamat ?? '-' }}</span>
                        </li>
                        <li class="flex items-start gap-3 group">
                            <div class="w-10 h-10 rounded-xl bg-gray-800 flex items-center justify-center text-indigo-400 shrink-0 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
                            </div>
                            <span class="pt-2">{{ $kontak->telepon ?? '-' }}</span>
                        </li>
                        <li class="flex items-start gap-3 group">
                            <div class="w-10 h-10 rounded-xl bg-gray-800 flex items-center justify-center text-indigo-400 shrink-0 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                            </div>
                            <span class="pt-2">{{ $kontak->email ?? '-' }}</span>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    el.classList.add('reveal-visible' + (dir ? '-' + dir : ''));
